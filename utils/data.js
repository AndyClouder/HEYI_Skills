...
const stats = {
  ...
  bySource: {
    skillhub: defaultSkills.length
  },
  byGate: {
    观: defaultSkills.filter(s => s.gate === '观').length,
    思: defaultSkills.filter(s => s.gate === '思').length,
    书: defaultSkills.filter(s => s.gate === '书').length,
    造: defaultSkills.filter(s => s.gate === '造').length,
    行: defaultSkills.filter(s => s.gate === '行').length,
    和: defaultSkills.filter(s => s.gate === '和').length
  }
};
...
function getSkillsByGate(gateId) {
  return defaultSkills.filter(s => s.gate === gateId);
}

function getSkillByCode(code) {
  return defaultSkills.find(s => s.code === code);
}
